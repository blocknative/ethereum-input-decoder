...
test('decoder', (t) => {
  ...
  t.test('Decoding a zero argument method call (deposit() in erc20)', (t) => {
    t.plan(1)
    ...
    t.deepEquals(result, expectedDeposit)
  })
  t.test('Decoding another zero argument method call (shutdown() in convex finance boost)', (t) => {
    t.plan(1)

    const decoder = new InputDataDecoder(`${__dirname}/data/convex_booster_abi.json`)
    const data = '0x354af919'
    const result = decoder.decodeData(data)
    const expectedShutdown = {
      "methodName": "shutdownSystem",
      "params": {}
    }
    console.log(JSON.stringify(expectedShutdown, 0, 2))
    t.deepEquals(result, expectedShutdown)
  })
